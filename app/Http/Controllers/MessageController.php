...
class MessageController extends Controller
{
    ...
    public function fetchMessages(Request $request, Order $order)
    {
        ...
        // Hancurkan Notifikasi Hantu: Ubah status menjadi "Read"
        Message::where('order_id', $order->id)->where('sender_id', '!=', auth()->id())
            ->where('is_read', false)->update(['is_read' => true]);

        // Tarik pesan beserta relasi Reply-nya
        $messages = Message::with('replyTo')->where('order_id', $order->id)->orderBy('created_at')->get();
    ...
    // --- PERBAIKAN FATAL: GENERAL CHAT CS ---
    ...
    // ==============================================================
    public function fetchGroupMessages(\App\Models\ChatGroup $group) {
        request()->session()->save();
        if (!auth()->check()) return response()->json(['error' => 'Unauthorized'], 401);
    ...
    public function sendGroupMessage(Request $request, \App\Models\ChatGroup $group) {
        if (!auth()->check()) return response()->json(['error' => 'Unauthorized'], 401);
